...
setup_config() {
  ...
  if command -v dos2unix &> /dev/null; then
    dos2unix "${NEOFETCH_CONFIG_DIR}/config.conf" 2>/dev/null
  elif command -v sed &> /dev/null; then
    sed -i 's/\r$//' "${NEOFETCH_CONFIG_DIR}/config.conf" 2>/dev/null
  else
    tr -d '\r' < "${NEOFETCH_CONFIG_DIR}/config.conf" > "${NEOFETCH_CONFIG_DIR}/config.conf.tmp" && mv "${NEOFETCH_CONFIG_DIR}/config.conf.tmp" "${NEOFETCH_CONFIG_DIR}/config.conf"
  fi
...
remove_setup() {
  ...
  if command -v neofetch &> /dev/null; then
    ...
    if command -v apt-get &> /dev/null; then
      ...
      sudo yum remove -y neofetch
    elif command -v dnf &> /dev/null; then
      sudo dnf remove -y neofetch
    elif command -v pacman &> /dev/null; then
      sudo pacman -R --noconfirm neofetch
    elif command -v zypper &> /dev/null; then
      sudo zypper remove -y neofetch
    else
      echo -e "${YELLOW}⚠️  Package manager not supported, please remove neofetch manually${RESET}"
    fi
